...
setLoading(false);
setMessage(`字符 "${selectedCharacter}" 載入成功！點擊「開始書寫練習」開始手寫。`);
},
onLoadCharDataError: () => {
setLoading(false);
...
<span className="text-2xl font-medium text-green-600 bg-green-50 px-4 py-2 rounded-full">
  {characterData[selectedCharacter]}
</span>
<button
  onClick={() => speakCharacter(selectedCharacter)}
  className="p-3 text-green-500 hover:text-green-700 hover:bg-green-100 rounded-full transition-all duration-200 shadow-md"
  title="點擊發音"
>
...
<svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-green-500 mr-3 mt-0.5 flex-shrink-0" viewBox="0 0 20 20" fill="currentColor">
